...
if __name__ == "__main__":
    ...
    print("EPOCHS=%d" % EPOCHS, file=OUT_FILE)
    print("BATCH_SIZE=%d" % BATCH_SIZE, file=OUT_FILE)
    print("WEIGHT_DECAY=%s" % WEIGHT_DECAY, file=OUT_FILE)
    print("LEARNING_RATE=%s" % LEARNING_RATE, file=OUT_FILE)
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        print("Epoch %i" % epoch, file=OUT_FILE)
        test_with_train_set(train_dataloader, model, args.show_data)
        test(test_dataloader, model, args.show_data)
...
def test_with_train_set(dataloader, model, show_data):
    if show_data:
    ...
    print(
        "Accuracy of the network on the 50000 train images: %d %%"
        % (100 * correct / total),
        file=OUT_FILE,
    )
    print(file=OUT_FILE)
